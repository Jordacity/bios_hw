...
## CAB: can't define 14 and 15 (above) until columns 9:13 are created
haart[,14] <- as.numeric(haart[,12]-haart[,9])
haart[,15] <- 0 
dim(haart[which(haart[,14]<365),])
```
...
## CAB: your code should display this! try this:
reg.cnts <- colSums(reg_drugs)
all_drugs[reg.cnts > 100]
```
...
## CAB: missing the end of the statement, fixed below
haart[i,16] <- min(as.numeric(haart[i,10] - haart[i,9]), as.numeric(haart[i,12] - haart[i, 9]),na.rm=T)}
...
## CAB: split on two or more spaces
split <- unlist(strsplit(addr[i,],split="[ ]{2,}")) 
...
I decided to take 10 points off for tardiness. I appreciate you letting me know the situation.

I considered taking points off for not matching your answers with the questions. You should submit homework with the up-to-date assignment.  Git and GitHub can be really useful tools when used correctly. If your repo is finicky, then we need to figure out why.

# Question 1, -3 points

I had to modify a few lines for the code to run. -2 points

1.5 Use R to create the output instead of manually typing the drug names.

1.6 Question requested first 5 and last 5 rows out data. -1 point

# Question 3, -4 points

Initial split by space failed. Using a single space failed to correctly parse name, street, and city when those fields contained spaces. -4 points

```{r}
# this creates 9 values
Filter(function(i) i != '', strsplit(addr[1,],split=" ")[[1]])
# this creates 6, as desired
Filter(function(i) i != '', strsplit(addr[1,],split="[ ]{2,}")[[1]])
```

You should also consider trimming the output.  For instance you have some zip codes that end with a space.

```{r}
as.character(address[1,'zip'])
```

# Question 4, -2 points, +5 points

You answered the bonus question, but failed to answer what you think causes the error. -2 points
